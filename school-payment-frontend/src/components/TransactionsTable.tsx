...
// This is a single row in our table. We make it a separate component
// to handle the advanced hover effect logic.
const TableRow = ({ transaction }: { transaction: Transaction }) => {
  const rowRef = useRef<HTMLTableRowElement>(null);

  // This function tracks the mouse position over the row and updates our CSS variables
  ...
  return (
    ...
    // We add the ref, the onMouseMove handler, and the "fancy-row" class
    <tr ref={rowRef} onMouseMove={handleMouseMove} className="fancy-row group">
      <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-800">{transaction.collect_id}</td>
